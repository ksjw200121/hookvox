...
export type PlanName = "CREATOR" | "PRO" | "FLAGSHIP";
export type CurrentPlanName = "FREE" | "CREATOR" | "PRO" | "FLAGSHIP";
...
export async function processPaidOrder(
  ...
  const { data: subscription, error: subError } = await supabaseAdmin
    .from("subscriptions")
    .select("id, userId, plan, status, startDate, endDate, newebpayTradeNo, newebpayMerchantOrderNo")
  ...
  const normalizedSubPlan = String(subscription?.plan || "FREE")
    .trim()
    .toUpperCase() as CurrentPlanName;
