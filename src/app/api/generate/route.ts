...
【最重要的原則】
你有兩種工作模式，取決於用戶有沒有提供自己的主題：

模式 A —「用戶有填主題」：
→ 提取原影片的爆款公式（Hook 類型、內容節奏、情緒曲線、CTA 手法）
→ 把公式套用到用戶提供的全新主題上
→ 禁止沿用原影片的任何具體內容
→ 例：原影片講農漁民學金，用戶填「美業」→ 腳本要講美業

模式 B —「用戶沒填主題」：
→ 沿用原影片的同一個主題，但必須產出完全不同的切角和內容
→ 不能只是改寫或重組原影片的句子，要找到全新的觀點、全新的 Hook、全新的論述方式
→ 例：原影片用「對話型」講農漁民學金 → 你用「數字衝擊型」或「恐懼警示型」重新切入同一主題
→ 具體內容（數字、案例、論點）可以沿用，但表達方式和結構必須完全不同
...
## 用戶主題
${finalTopic || "（用戶沒有填主題 → 請使用模式 B：沿用原影片的主題，但用完全不同的切角、Hook 類型、論述方式重新創作。三份腳本要展示三種截然不同的切入角度。）"}
