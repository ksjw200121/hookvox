...
【最重要的原則】
你的工作不是「改寫原影片」，而是「提取爆款公式，套用到全新的主題」。
就像學會數學解題方法後去解新的題目，不是抄別人的答案。
生成的腳本必須是全新的內容，只保留原影片的「結構和手法」，不保留「具體內容和案例」。
...
## 用戶主題
${finalTopic || "（用戶未提供主題。請根據爆款公式的結構和邏輯，自行挑選一個相同行業但不同的具體主題來生成全新腳本。絕對不要重複或改寫原影片的內容，要用全新的主角和案例。）"}
